...
|---|---|---|---|---|
| 친구 신청 | `FRIEND_REQUEST` | `@inning님이 친구 신청을 보냈습니다.` | `requestId`, `actorUserId`, `deepLink` | 도메인 구현 필요 |
| 친구 신청 | `FRIEND_ACCEPTED` | `@inning님이 친구신청을 수락했습니다.` | `friendshipId`, `actorUserId`, `deepLink` | 도메인 구현 필요 |
| 댓글 또는 반응 | `TIMELINE_REACTION` | `@inning님이 회원님의 기록에 반응했습니다.` | `timelineId`, `actorUserId`, `reactionType`, `deepLink` | MVP 제안, 도메인 구현 필요 |
| 댓글 또는 반응 | `TIMELINE_COMMENT` | `@inning님이 댓글을 달았습니다.` | `timelineId`, `commentId`, `actorUserId`, `deepLink` | Figma 메모상 댓글은 후순위 |
| 경기 진행 상황 | `GAME_INNING_STARTED` | `1회가 시작되었습니다!` | `gameId`, `inning`, `half`, `deepLink` | 경기 feed/도메인 필요 |
| 경기 진행 상황 | `GAME_INNING_ENDED` | `1회가 종료되었습니다.` | `gameId`, `inning`, `half`, `deepLink` | 경기 feed/도메인 필요 |
| 경기 진행 상황 | `GAME_SCORE_CHANGED` | `응원 팀의 점수가 변경되었습니다.` | `gameId`, `homeScore`, `awayScore`, `deepLink` | 경기 feed/도메인 필요 |
| 기록 독촉 | `RECORD_REMINDER` | `9회가 끝나기 전에 기록해주세요.` | `gameId`, `inning`, `deepLink` | 기록/스케줄러 도메인 필요 |
| 비동기 결과 | `GENERATED_VIDEO_READY` | 생성 영상 준비 완료 | `videoId`, `deepLink` | 영상 작업 도메인 필요 |
| 비동기 결과 | `GENERATED_VIDEO_FAILED` | 생성 영상 처리 실패 | `videoId`, `deepLink` | 영상 작업 도메인 필요 |
...
- 인증 사용자별 FCM 등록/비활성화 API
- `deviceId=FID`, `pushToken=FCM registration token` 계약
- 동일 설치에서 새 사용자가 로그인하면 설치 소유권 이전
- 이전 사용자의 늦은 DELETE가 새 소유자의 등록을 비활성화하지 않는 소유권 검사
- DB 잠금과 전역 `device_id` unique를 통한 동시 등록 안정화
- 적용 전 중복 `device_id`를 최신 `last_seen_at` 등록 우선으로 정리하는 Flyway V11
...
- token rotation 후 도착한 과거 invalid 응답이 새 토큰을 끄지 않는 조건부 정리
- 4,096 UTF-8 byte payload 제한과 FCM 예약 data key 검사
- token/payload 원문을 제외한 구조화 로그와 bounded-tag 메트릭
- 계정 탈퇴 시 사용자의 push 등록 일괄 비활성화

이 목록은 코드 상태를 뜻한다. Firebase Android/iOS 앱 등록, 플랫폼 설정 파일 생성, APNs key 업로드, AWS 실제 배포, 실기기 종단 FCM smoke test는 아직 완료되지 않았다.
...
사용자별 등록 기기 수 제한과 등록 API rate limit은 요청에 따라 이번 범위에서는 보류한다. 인증 사용자가 많은 임의 FID/token을 등록할 수 있는 잔여 위험은 운영 전 별도 보안 항목으로 추적한다.

토큰 PUT/DELETE의 교차 충돌을 확실히 막기 위해 현재는 단일 DB registration lock으로 직렬화한다. 같은 FID/token 경합과 계정 전환의 정확성은 보장하지만 서로 무관한 설치의 등록도 잠깐 대기하므로, 등록량이 커지거나 서버를 수평 확장하기 전에는 PostgreSQL key-scoped advisory lock 또는 동등한 원자 reconciliation으로 교체하고 부하 테스트해야 한다.
    ...
    notificationQueueService.enqueueToUser(
            ...
            new PushNotification(
                    ...
                    Map.of(
                            ...
                            "deepLink", "inninglog://friends/" + acceptingUserId)));
}
```
...
- FCM 네트워크, Firebase 인증, 개별 token 오류는 핵심 비즈니스 트랜잭션에 전파되지 않는다.
...
도메인 발송자는 여기에 entity ID와 `deepLink`를 추가한다. 모든 값은 문자열이어야 한다.
...
재시도 대상에는 `UNAVAILABLE`, `INTERNAL`, `QUOTA_EXCEEDED`, platform `DEADLINE_EXCEEDED`, `RESOURCE_EXHAUSTED`, 알 수 없는 전체 batch 실패가 포함된다. 자동 token 비활성화는 오등록을 대량으로 끌 위험이 가장 낮은 `UNREGISTERED`에만 적용한다. `INVALID_ARGUMENT`, `SENDER_ID_MISMATCH`는 terminal 실패로 관측하고 설정을 점검한다.
...
5. 실제 테스트 기기의 token 등록 후 각 플랫폼 smoke test
